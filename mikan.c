...
void mikan_init(
	...
	for (int l = 0; l < KAN_NUM_LAYERS - 1; ++l) {
		for (int j = 0; j < num_nodes[l + 1]; ++j) {
			for (int i = 0; i < num_nodes[l]; ++i) {
				emlp_init(emlp_num_nodes, emlp_weight[l][j][i], emlp_bias[l][j][i]);
			}
		}
	}
...
void mikan_forward(
	...
	for (int l = 0; l < KAN_NUM_LAYERS - 1; ++l) {
		...
		for (int j = 0; j < num_nodes[l + 1]; ++j) {
			out[l + 1][j] = 0;
			for (int i = 0; i < num_nodes[l]; ++i) {
				emlp_forward(out[l][i], emlp_num_nodes, emlp_weight[l][j][i], emlp_bias[l][j][i], emlp_out[l][j][i]);
...
void mikan_backprop(
	...
	for (int l = KAN_NUM_LAYERS - 2; l > 0; --l) {
		...
		for (int i = 0; i < num_nodes[l]; ++i) {
			...
			for (int j = 0; j < num_nodes[l + 1]; ++j) {
				emlp_backprop(delta[l + 1][j], emlp_num_nodes, emlp_weight[l][j][i], emlp_bias[l][j][i], emlp_out[l][j][i], emlp_delta[l][j][i]);
